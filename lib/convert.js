...
TiffConverter.prototype.convert = function(){
  ...
  var filename = path.basename(original, path.extname(original)),
    target = _this.options.saveInPlace ? path.dirname(original) + '/' + prefix + filename + suffix + '.' + type : _this.location + '/' + filename + '/' + prefix + '1' + suffix;
  ...
  _this.createDir(target, filename, function(err){
    ...
    //command += ' ' + original + ' ' + target;
    command += ' ' + original + ' -scene 1 ' + _this.location + '/' + filename + '/' + prefix + '%d' + suffix + '.' + type;
