...
/**
 * Options:
 * type: 'png' (default), 'jpg', 'gif'
 * logLevel: 1 = info, 0 = errors only (default)
 * prefix: '' (optional)
 * suffix: '' (optional)
 * tmpPath: null (optional)
 * autoRemoveTmp: true, false (default)
 * commandPath: null (optional)
 * saveInPlace: true, false (default)
 */
...
var TiffConverter = function(options){
  ...
  if (options) {
    ...
    _this.options.saveInPlace = options.saveInPlace || false;
  }
...
TiffConverter.prototype.convert = function(){
  ...
  var filename = path.basename(original, path.extname(original)),
    target = _this.options.saveInPlace ? path.dirname(original) + '/' + prefix + filename + suffix : _this.location + '/' + filename + '/' + prefix + '1' + suffix;

  if (_this.options.saveInPlace) {
      runCommand();
  } else {
      _this.createDir(target, filename, runCommand);
  }
